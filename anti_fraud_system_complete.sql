-- =====================================================
-- COMPLETE ANTI-FRAUD SYSTEM FOR AFFILIATE PLATFORM
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_affiliate_devices_affiliate ON affiliate_devices(affiliate_id);
...
-- Index for IP and affiliate lookups
CREATE INDEX IF NOT EXISTS idx_video_clicks_ip ON video_clicks(ip_address);
CREATE INDEX IF NOT EXISTS idx_video_clicks_affiliate ON video_clicks(affiliate_id);
...
CREATE INDEX IF NOT EXISTS idx_affiliate_penalties_affiliate ON affiliate_penalties(affiliate_id);
...
CREATE INDEX IF NOT EXISTS idx_admin_reviews_affiliate ON admin_task_reviews(affiliate_id);
...
ADD COLUMN IF NOT EXISTS affiliate_status TEXT DEFAULT 'active' CHECK (affiliate_status IN ('active', 'suspended', 'banned')),
...
-- Function: Check if IP is shared by multiple affiliates
CREATE OR REPLACE FUNCTION check_ip_shared(p_ip TEXT, p_affiliate_id UUID)
RETURNS TABLE(shared_affiliate_id UUID, ip_address TEXT, click_count INT) AS $$
BEGIN
    RETURN QUERY
    SELECT DISTINCT vc.affiliate_id, vc.ip_address, COUNT(vc.id)::INT as click_count
    FROM video_clicks vc
    WHERE vc.ip_address = p_ip
    AND vc.affiliate_id != p_affiliate_id
    GROUP BY vc.affiliate_id, vc.ip_address;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function: Check if device is shared
CREATE OR REPLACE FUNCTION check_device_shared(p_fingerprint TEXT, p_affiliate_id UUID)
RETURNS TABLE(shared_affiliate_id UUID, fingerprint TEXT, device_count INT) AS $$
BEGIN
    RETURN QUERY
    SELECT DISTINCT ad.affiliate_id, ad.device_fingerprint, COUNT(ad.id)::INT as device_count
    FROM affiliate_devices ad
    WHERE ad.device_fingerprint = p_fingerprint
    AND ad.affiliate_id != p_affiliate_id
    GROUP BY ad.affiliate_id, ad.device_fingerprint;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DECLARE
    ...
    v_shared_affiliates INT := 0;
    v_shared_devices INT := 0;
    v_ip_count INT;
    v_device_count INT;
BEGIN
    -- Check IP sharing
    SELECT COUNT(DISTINCT affiliate_id) INTO v_ip_count
    FROM video_clicks
    ...
    IF v_ip_count > 0 THEN
        ...
        v_ip_shared := true;
        v_shared_affiliates := v_ip_count;
    END IF;

    -- Check device sharing
    SELECT COUNT(DISTINCT affiliate_id) INTO v_device_count
    ...
    IF v_device_count > 0 THEN
        ...
        -- If 3+ affiliates on same device = automatic fraud
    ...
    -- Return results
    RETURN QUERY SELECT v_score, v_flags, v_ip_shared, v_device_shared, v_ua_match, v_shared_affiliates, v_shared_devices;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function: Apply penalty to affiliate
CREATE OR REPLACE FUNCTION apply_affiliate_penalty(
    p_affiliate_id UUID,
...
-- Function: Register device for affiliate
CREATE OR REPLACE FUNCTION register_affiliate_device(
    p_affiliate_id UUID,
    p_fingerprint TEXT,
    p_ip TEXT,
    p_user_agent TEXT
...
DECLARE
    ...
    v_existing_id UUID;
BEGIN
    -- Check if device already exists for this affiliate
...
-- =====================================================
CREATE OR REPLACE VIEW v_affiliate_fraud_analysis AS
SELECT
...
FROM profiles p
LEFT JOIN affiliate_devices ad ON ad.affiliate_id = p.id
LEFT JOIN video_clicks vc ON vc.affiliate_id = p.id
LEFT JOIN affiliate_penalties ap ON ap.affiliate_id = p.id
